...
前置きが長くなったが，いよいよ意味計算を Haskell の上で行っていく．まずは準備として，対象言語を Haskell に埋め込んだ eDSL である `Model` 言語を導入する．
...
- `Model` 言語は，対象言語を Haskell に埋め込んだ eDSL である．
...
`is_a` 関数を使えば，文 $\textit{Linus is a boy.}$ は以下のように Haskell に埋め込むことができる．
